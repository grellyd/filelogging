...
// Logger which can log to disk and std.Out
type Logger struct {
	name  string
	log   *log.Logger
	file  *os.File
...
// NewLogger creates a new logger that may log to disk
func NewLogger(loggerName string, state state.State) (logger *Logger, err error) {
...
func GetLogger(loggerName string) (logger *Logger) {
	...
	logger, err := NewLogger(loggerName, state.NORMAL)
	if err != nil {
